...
class BoardMapper extends QBMapper implements IPermissionMapper {
	...
	public function findAllByUser(string $userId, ?int $limit = null, ?int $offset = null, ?int $since = null,
		...
		$qb = $this->db->getQueryBuilder();
		$qb->select('id', 'title', 'owner', 'color', 'archived', 'deleted_at', 'last_modified', 'category_id')
		...
		// shared with user
		$qb->resetQueryParts();
		$qb->select('b.id', 'title', 'owner', 'color', 'archived', 'deleted_at', 'last_modified', 'category_id')
