...
class BoardApiController extends ApiController {
	 ...
	 */
	public function update($title, $color, $archived = false, $categoryId = null) {
		$board = $this->boardService->update($this->request->getParam('boardId'), $title, $color, $archived, $categoryId);
		return new DataResponse($board, HTTP::STATUS_OK);
	}
